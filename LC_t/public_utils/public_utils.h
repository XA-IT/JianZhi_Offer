...
// 根据数组构建链表，返回头节点
inline ListNode *buildListFromVec(vector<int> &nums)
{
    int sz = nums.size();
    if (sz == 0)
        return NULL;
...
// 按前序遍历打印所有节点的值
inline void printPreorderBinTree(TreeNode *root)
{
    if (root == NULL)
    {
        return;
...
inline void printBFSBinTree(TreeNode *root)
{
    if (root == NULL)
        return;
    queue<TreeNode *> qu;
...
// 根据层序遍历的数组构建二叉树，返回根节点
// 空节点值为-2
inline TreeNode *buildTreeFromBFSVec(vector<int> &nums)
{
    int sz = nums.size();
    if (sz == 0)
    {
...
inline vector<vector<char>> transIntMatrix2Chars(vector<vector<int>> &nums)
{
    int row = nums.size();
    if (row < 1)
        return { };
...
// 打印矩阵的内容
inline void printMatrix(vector<vector<int>>& nums)
{
    int row = nums.size();
    if (row < 1)
        return;
...
inline void printMatrix(vector<vector<char>>& nums)
{
    int row = nums.size();
    if (row < 1)
        return;
